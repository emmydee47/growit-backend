 ...
 * Page class
 */
class ArticlesList extends Articles
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ArticlesList";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["articles"]) || get_class($GLOBALS["articles"]) == PROJECT_NAMESPACE . "articles") {
        ...
        // Initialize URLs
        $this->AddUrl = "articlesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "articlesdelete";
        $this->MultiUpdateUrl = "articlesupdate";
    ...
    public function run()
    {
        ...
        $this->usefull_links->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->usefull_links->AdvancedSearch->toJson(), ","); // Field usefull_links
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "farticlessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field usefull_links
        $this->usefull_links->AdvancedSearch->SearchValue = @$filter["x_usefull_links"];
        $this->usefull_links->AdvancedSearch->SearchOperator = @$filter["z_usefull_links"];
        $this->usefull_links->AdvancedSearch->SearchCondition = @$filter["v_usefull_links"];
        $this->usefull_links->AdvancedSearch->SearchValue2 = @$filter["y_usefull_links"];
        $this->usefull_links->AdvancedSearch->SearchOperator2 = @$filter["w_usefull_links"];
        $this->usefull_links->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->article_category_id); // article_category_id
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"farticleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"farticleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"farticlessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"farticlessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="farticleslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->usefull_links->setDbValue($row['usefull_links']);
    ...
    protected function newRow()
    {
        ...
        $row['usefull_links'] = $this->usefull_links->DefaultValue;
        $row['media_url'] = $this->media_url->DefaultValue;
        $row['article_category_id'] = $this->article_category_id->DefaultValue;
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->article_category_id->ViewValue === null) { // Lookup from database
                    ...
                    $sqlWrk = $this->article_category_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
            ...
            if (!EmptyValue($this->media_url->Upload->DbValue)) {
                $this->media_url->HrefValue = GetFileUploadUrl($this->media_url, $this->media_url->htmlDecode($this->media_url->Upload->DbValue)); // Add prefix/suffix
            ...
            if ($this->media_url->UseColorbox) {
                ...
                $this->media_url->LinkAttrs["data-rel"] = "articles_x" . $this->RowCount . "_media_url";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"farticlessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
