 ...
 * Page class
 */
class RemindersAdd extends Reminders
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RemindersAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("reminderslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "reminderslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "remindersview") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->reminder_date->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->reminder_date->setFormValue($val, true, $validate);
            }
            $this->reminder_date->CurrentValue = UnFormatDateTime($this->reminder_date->CurrentValue, $this->reminder_date->formatPattern());
        }
    ...
    public function renderRow()
    {
        ...
        // reminder_date
        $this->reminder_date->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // reminder_date
            $this->reminder_date->setupEditAttributes();
            $this->reminder_date->EditCustomAttributes = "";
            $this->reminder_date->EditValue = HtmlEncode(FormatDateTime($this->reminder_date->CurrentValue, $this->reminder_date->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        if ($this->reminder_date->Required) {
            if (!$this->reminder_date->IsDetailKey && EmptyValue($this->reminder_date->FormValue)) {
                $this->reminder_date->addErrorMessage(str_replace("%s", $this->reminder_date->caption(), $this->reminder_date->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->reminder_date->FormValue, $this->reminder_date->formatPattern())) {
            $this->reminder_date->addErrorMessage($this->reminder_date->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // reminder_date
        $this->reminder_date->setDbValueDef($rsnew, UnFormatDateTime($this->reminder_date->CurrentValue, $this->reminder_date->formatPattern()), null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("reminderslist"), "", $this->TableVar, true);
