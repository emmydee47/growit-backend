...
/**
 * Table class for crop_steps
 */
class CropSteps extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'crop_steps';
        $this->TableName = 'crop_steps';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "\"crop_steps\"";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->step_type->InputTextType = "text";
        $this->step_type->Nullable = false; // NOT NULL field
        $this->step_type->Required = true; // Required field
        $this->step_type->Lookup = new Lookup('step_type', 'crop_steps', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->step_type->OptionCount = 3;
        $this->Fields['step_type'] = &$this->step_type;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "\"crop_steps\"";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->step_type->DbValue = $row['step_type'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("cropstepslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "cropstepsview") {
            return $Language->phrase("View");
        } elseif ($pageName == "cropstepsedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "cropstepsadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "CropStepsView";
            case Config("API_ADD_ACTION"):
                return "CropStepsAdd";
            case Config("API_EDIT_ACTION"):
                return "CropStepsEdit";
            case Config("API_DELETE_ACTION"):
                return "CropStepsDelete";
            case Config("API_LIST_ACTION"):
                return "CropStepsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("cropstepsview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("cropstepsview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "cropstepsadd?" . $this->getUrlParm($parm);
        } else {
            $url = "cropstepsadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("cropstepsedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("cropstepsadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("cropstepsdelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        // step_type
        $this->step_type->EditCustomAttributes = "";
        $this->step_type->EditValue = $this->step_type->options(false);
        $this->step_type->PlaceHolder = RemoveHtml($this->step_type->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->crop_id);
                    $doc->exportCaption($this->step_type);
                    $doc->exportCaption($this->_title);
                    $doc->exportCaption($this->updated_at);
                    $doc->exportCaption($this->created_at);
                }
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->id);
                        $doc->exportField($this->crop_id);
                        $doc->exportField($this->step_type);
                        $doc->exportField($this->_title);
                        $doc->exportField($this->updated_at);
                        $doc->exportField($this->created_at);
                    }
