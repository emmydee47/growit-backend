 ...
 * Page class
 */
class CropStepsList extends CropSteps
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'crop_steps';

    // Page object name
    public $PageObjName = "CropStepsList";
    ...
    public $FormName = "fcrop_stepslist";
    ...
    public function __construct()
    {
        ...
        // Table object (crop_steps)
        if (!isset($GLOBALS["crop_steps"]) || get_class($GLOBALS["crop_steps"]) == PROJECT_NAMESPACE . "crop_steps") {
            $GLOBALS["crop_steps"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "cropstepsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "cropstepsdelete";
        $this->MultiUpdateUrl = "cropstepsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'crop_steps');
        }
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->step_type);
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcrop_stepssrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->step_type); // step_type
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canAdd()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"crop_steps\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcrop_stepslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcrop_stepslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if (IsMobile()) {
            ...
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"crop_steps\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("step_type", $this->createColumnOption("step_type"));
            $option->add("title", $this->createColumnOption("title"));
            $option->add("content", $this->createColumnOption("content"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcrop_stepssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcrop_stepssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcrop_stepslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->step_type->setDbValue($row['step_type']);
    ...
    protected function newRow()
    {
        ...
        $row['step_type'] = $this->step_type->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // step_type
            if (strval($this->step_type->CurrentValue) != "") {
                $this->step_type->ViewValue = $this->step_type->optionCaption($this->step_type->CurrentValue);
            } else {
                $this->step_type->ViewValue = null;
            }
            $this->step_type->ViewCustomAttributes = "";
            ...
            // step_type
            $this->step_type->LinkCustomAttributes = "";
            $this->step_type->HrefValue = "";
            $this->step_type->TooltipValue = "";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcrop_stepssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
