...
class ArticlesAdd extends Articles
{
    ...
    public function run()
    {
        ...
        $this->usefull_links->setVisibility();
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'usefull_links' first before field var 'x_usefull_links'
        $val = $CurrentForm->hasValue("usefull_links") ? $CurrentForm->getValue("usefull_links") : $CurrentForm->getValue("x_usefull_links");
        if (!$this->usefull_links->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->usefull_links->Visible = false; // Disable update for API request
            } else {
                $this->usefull_links->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->usefull_links->CurrentValue = $this->usefull_links->FormValue;
    ...
    public function renderRow()
    {
        ...
        // usefull_links
        $this->usefull_links->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // usefull_links
            $this->usefull_links->ViewValue = $this->usefull_links->CurrentValue;
            $this->usefull_links->ViewCustomAttributes = "";
            ...
            // usefull_links
            $this->usefull_links->LinkCustomAttributes = "";
            $this->usefull_links->HrefValue = "";
            ...
            // usefull_links
            $this->usefull_links->setupEditAttributes();
            $this->usefull_links->EditCustomAttributes = "";
            $this->usefull_links->EditValue = HtmlEncode($this->usefull_links->CurrentValue);
            $this->usefull_links->PlaceHolder = RemoveHtml($this->usefull_links->caption());
            ...
            if ($this->article_category_id->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "\"id\"" . SearchString("=", $this->article_category_id->CurrentValue, DATATYPE_GUID, "");
                }
                $sqlWrk = $this->article_category_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function validateForm()
    {
        ...
        if ($this->usefull_links->Required) {
            if (!$this->usefull_links->IsDetailKey && EmptyValue($this->usefull_links->FormValue)) {
                $this->usefull_links->addErrorMessage(str_replace("%s", $this->usefull_links->caption(), $this->usefull_links->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // usefull_links
        $this->usefull_links->setDbValueDef($rsnew, $this->usefull_links->CurrentValue, "", false);
        ...
        if ($this->media_url->Visible && !$this->media_url->Upload->KeepFile) {
            ...
            $this->media_url->ImageWidth = Config("THUMBNAIL_DEFAULT_WIDTH"); // Resize width
            $this->media_url->ImageHeight = Config("THUMBNAIL_DEFAULT_HEIGHT"); // Resize height
        }

        // article_category_id
        $this->article_category_id->setDbValueDef($rsnew, $this->article_category_id->CurrentValue, null, false);
                        ...
                        for ($i = 0; $i < $newFileCount; $i++) {
                            if ($newFiles[$i] != "") {
                                ...
                                if (file_exists($file)) {
                                    ...
                                    if (!$this->media_url->Upload->ResizeAndSaveToFile($this->media_url->ImageWidth, $this->media_url->ImageHeight, 100, $newFiles[$i], true, $i)) {
