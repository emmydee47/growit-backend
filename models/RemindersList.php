 ...
 * Page class
 */
class RemindersList extends Reminders
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RemindersList";
    ...
    public $FormName = "freminderslist";
    ...
    public function __construct()
    {
        ...
        // Table object (reminders)
        if (!isset($GLOBALS["reminders"]) || get_class($GLOBALS["reminders"]) == PROJECT_NAMESPACE . "reminders") {
            $GLOBALS["reminders"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "remindersadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "remindersdelete";
        $this->MultiUpdateUrl = "remindersupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'reminders');
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->reminder_date->AdvancedSearch->toJson(), ","); // Field reminder_date
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "freminderssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field reminder_date
        $this->reminder_date->AdvancedSearch->SearchValue = @$filter["x_reminder_date"];
        $this->reminder_date->AdvancedSearch->SearchOperator = @$filter["z_reminder_date"];
        $this->reminder_date->AdvancedSearch->SearchCondition = @$filter["v_reminder_date"];
        $this->reminder_date->AdvancedSearch->SearchValue2 = @$filter["y_reminder_date"];
        $this->reminder_date->AdvancedSearch->SearchOperator2 = @$filter["w_reminder_date"];
        $this->reminder_date->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->reminder_date); // reminder_date
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"freminderslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"freminderslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("reminder_date", $this->createColumnOption("reminder_date"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"freminderssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"freminderssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="freminderslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['reminder_date'] = $this->reminder_date->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->reminder_date->ViewValue = FormatDateTime($this->reminder_date->ViewValue, $this->reminder_date->formatPattern());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"freminderssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
