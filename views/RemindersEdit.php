...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fremindersedit = new ew.Form("fremindersedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fremindersedit;

    // Add fields
    var fields = currentTable.fields;
    fremindersedit.addFields([
    ...
    // Form_CustomValidate
    fremindersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fremindersedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fremindersedit.lists.user_id = <?= $Page->user_id->toClientList($Page) ?>;
    loadjs.done("fremindersedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fremindersedit" id="fremindersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="reminders" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_reminders_user_id">
    <select
        ...
        data-select2-id="fremindersedit_x_user_id"
...
<script>
loadjs.ready("fremindersedit", function() {
    var options = { name: "x_user_id", selectId: "fremindersedit_x_user_id" };
    if (fremindersedit.lists.user_id.lookupOptions.length) {
        options.data = { id: "x_user_id", form: "fremindersedit" };
    } else {
        options.ajax = { id: "x_user_id", form: "fremindersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fremindersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fremindersedit", "x_reminder_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fremindersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fremindersedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fremindersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fremindersedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
