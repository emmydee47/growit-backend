...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    farticlesedit = new ew.Form("farticlesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = farticlesedit;

    // Add fields
    var fields = currentTable.fields;
    farticlesedit.addFields([
    ...
    // Form_CustomValidate
    farticlesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticlesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    farticlesedit.lists.article_category_id = <?= $Page->article_category_id->toClientList($Page) ?>;
    loadjs.done("farticlesedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="farticlesedit" id="farticlesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="articles" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_articles_article_category_id">
    <select
        ...
        data-select2-id="farticlesedit_x_article_category_id"
        data-table="articles"
...
<script>
loadjs.ready("farticlesedit", function() {
    var options = { name: "x_article_category_id", selectId: "farticlesedit_x_article_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (farticlesedit.lists.article_category_id.lookupOptions.length) {
        options.data = { id: "x_article_category_id", form: "farticlesedit" };
    } else {
        options.ajax = { id: "x_article_category_id", form: "farticlesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["farticlesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("farticlesedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["farticlesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("farticlesedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
