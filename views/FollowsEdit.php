...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffollowsedit = new ew.Form("ffollowsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffollowsedit;

    // Add fields
    var fields = currentTable.fields;
    ffollowsedit.addFields([
    ...
    // Form_CustomValidate
    ffollowsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffollowsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    ffollowsedit.lists.user_id = <?= $Page->user_id->toClientList($Page) ?>;
    ffollowsedit.lists.follower = <?= $Page->follower->toClientList($Page) ?>;
    ffollowsedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("ffollowsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ffollowsedit" id="ffollowsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="follows" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_follows_user_id">
    <select
        ...
        data-select2-id="ffollowsedit_x_user_id"
        data-table="follows"
...
<script>
loadjs.ready("ffollowsedit", function() {
    var options = { name: "x_user_id", selectId: "ffollowsedit_x_user_id" };
    if (ffollowsedit.lists.user_id.lookupOptions.length) {
        options.data = { id: "x_user_id", form: "ffollowsedit" };
    } else {
        options.ajax = { id: "x_user_id", form: "ffollowsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_follows_follower">
    <select
        ...
        data-select2-id="ffollowsedit_x_follower"
        data-table="follows"
...
<script>
loadjs.ready("ffollowsedit", function() {
    var options = { name: "x_follower", selectId: "ffollowsedit_x_follower" };
    if (ffollowsedit.lists.follower.lookupOptions.length) {
        options.data = { id: "x_follower", form: "ffollowsedit" };
    } else {
        options.ajax = { id: "x_follower", form: "ffollowsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->status->isInvalidClass() ?>" data-table="follows" data-field="x_status" name="x_status[]" id="x_status_433219" value="1"<?= ConvertToBool($Page->status->CurrentValue) ? " checked" : "" ?><?= $Page->status->editAttributes() ?> aria-describedby="x_status_help">
...
<script>
loadjs.ready(["ffollowsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffollowsedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["ffollowsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffollowsedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
