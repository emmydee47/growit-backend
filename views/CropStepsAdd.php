...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcrop_stepsadd = new ew.Form("fcrop_stepsadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fcrop_stepsadd;

    // Add fields
    var fields = currentTable.fields;
    fcrop_stepsadd.addFields([
    ...
    // Form_CustomValidate
    fcrop_stepsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrop_stepsadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcrop_stepsadd.lists.crop_id = <?= $Page->crop_id->toClientList($Page) ?>;
    loadjs.done("fcrop_stepsadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcrop_stepsadd" id="fcrop_stepsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_crop_steps_crop_id">
    <select
        ...
        data-select2-id="fcrop_stepsadd_x_crop_id"
...
<script>
loadjs.ready("fcrop_stepsadd", function() {
    var options = { name: "x_crop_id", selectId: "fcrop_stepsadd_x_crop_id" };
    if (fcrop_stepsadd.lists.crop_id.lookupOptions.length) {
        options.data = { id: "x_crop_id", form: "fcrop_stepsadd" };
    } else {
        options.ajax = { id: "x_crop_id", form: "fcrop_stepsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fcrop_stepsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_stepsadd", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fcrop_stepsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_stepsadd", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
