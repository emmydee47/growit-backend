...
// Page object
$CropRecommendationsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { crop_recommendations: currentTable } });
var currentForm, currentPageID;
var fcrop_recommendationsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcrop_recommendationsedit = new ew.Form("fcrop_recommendationsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcrop_recommendationsedit;

    // Add fields
    var fields = currentTable.fields;
    fcrop_recommendationsedit.addFields([
    ...
    // Form_CustomValidate
    fcrop_recommendationsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrop_recommendationsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcrop_recommendationsedit.lists.crop_id = <?= $Page->crop_id->toClientList($Page) ?>;
    loadjs.done("fcrop_recommendationsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcrop_recommendationsedit" id="fcrop_recommendationsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="crop_recommendations" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_crop_recommendations_crop_id">
    <select
        ...
        data-select2-id="fcrop_recommendationsedit_x_crop_id"
...
<script>
loadjs.ready("fcrop_recommendationsedit", function() {
    var options = { name: "x_crop_id", selectId: "fcrop_recommendationsedit_x_crop_id" };
    if (fcrop_recommendationsedit.lists.crop_id.lookupOptions.length) {
        options.data = { id: "x_crop_id", form: "fcrop_recommendationsedit" };
    } else {
        options.ajax = { id: "x_crop_id", form: "fcrop_recommendationsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_thumbnail_url" id= "fa_x_thumbnail_url" value="<?= (Post("fa_x_thumbnail_url") == "0") ? "0" : "1" ?>">
...
<script>
loadjs.ready(["fcrop_recommendationsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_recommendationsedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fcrop_recommendationsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_recommendationsedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
