...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcrop_tipsedit = new ew.Form("fcrop_tipsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcrop_tipsedit;

    // Add fields
    var fields = currentTable.fields;
    fcrop_tipsedit.addFields([
    ...
    // Form_CustomValidate
    fcrop_tipsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrop_tipsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcrop_tipsedit.lists.crop_id = <?= $Page->crop_id->toClientList($Page) ?>;
    loadjs.done("fcrop_tipsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcrop_tipsedit" id="fcrop_tipsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="crop_tips" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_crop_tips_crop_id">
    <select
        ...
        data-select2-id="fcrop_tipsedit_x_crop_id"
...
<script>
loadjs.ready("fcrop_tipsedit", function() {
    var options = { name: "x_crop_id", selectId: "fcrop_tipsedit_x_crop_id" };
    if (fcrop_tipsedit.lists.crop_id.lookupOptions.length) {
        options.data = { id: "x_crop_id", form: "fcrop_tipsedit" };
    } else {
        options.ajax = { id: "x_crop_id", form: "fcrop_tipsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fcrop_tipsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_tipsedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fcrop_tipsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_tipsedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
