...
// Page object
$ArticleCategoriesEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { article_categories: currentTable } });
var currentForm, currentPageID;
var farticle_categoriesedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    farticle_categoriesedit = new ew.Form("farticle_categoriesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = farticle_categoriesedit;

    // Add fields
    var fields = currentTable.fields;
    farticle_categoriesedit.addFields([
    ...
    // Form_CustomValidate
    farticle_categoriesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    farticle_categoriesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("farticle_categoriesedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="farticle_categoriesedit" id="farticle_categoriesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="article_categories" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_image_url" id= "fa_x_image_url" value="<?= (Post("fa_x_image_url") == "0") ? "0" : "1" ?>">
...
<script>
loadjs.ready(["farticle_categoriesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("farticle_categoriesedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["farticle_categoriesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("farticle_categoriesedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
