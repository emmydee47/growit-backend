...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsuggestionsadd = new ew.Form("fsuggestionsadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fsuggestionsadd;

    // Add fields
    var fields = currentTable.fields;
    fsuggestionsadd.addFields([
    ...
    // Form_CustomValidate
    fsuggestionsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsuggestionsadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsuggestionsadd.lists.month = <?= $Page->month->toClientList($Page) ?>;
    fsuggestionsadd.lists.crop_id = <?= $Page->crop_id->toClientList($Page) ?>;
    loadjs.done("fsuggestionsadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsuggestionsadd" id="fsuggestionsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_suggestions_month">
    <select
        ...
        data-select2-id="fsuggestionsadd_x_month"
...
<script>
loadjs.ready("fsuggestionsadd", function() {
    var options = { name: "x_month", selectId: "fsuggestionsadd_x_month" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsuggestionsadd.lists.month.lookupOptions.length) {
        options.data = { id: "x_month", form: "fsuggestionsadd" };
    } else {
        options.ajax = { id: "x_month", form: "fsuggestionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_suggestions_crop_id">
    <select
        ...
        data-select2-id="fsuggestionsadd_x_crop_id"
        data-table="suggestions"
...
<script>
loadjs.ready("fsuggestionsadd", function() {
    var options = { name: "x_crop_id", selectId: "fsuggestionsadd_x_crop_id" };
    if (fsuggestionsadd.lists.crop_id.lookupOptions.length) {
        options.data = { id: "x_crop_id", form: "fsuggestionsadd" };
    } else {
        options.ajax = { id: "x_crop_id", form: "fsuggestionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fsuggestionsadd", "editor"], function() {
    ew.createEditor("fsuggestionsadd", "x__content", 35, 4, <?= $Page->_content->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fsuggestionsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsuggestionsadd", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fsuggestionsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsuggestionsadd", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
