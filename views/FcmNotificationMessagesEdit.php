...
// Page object
$FcmNotificationMessagesEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { fcm_notification_messages: currentTable } });
var currentForm, currentPageID;
var ffcm_notification_messagesedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ffcm_notification_messagesedit = new ew.Form("ffcm_notification_messagesedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ffcm_notification_messagesedit;

    // Add fields
    var fields = currentTable.fields;
    ffcm_notification_messagesedit.addFields([
    ...
    // Form_CustomValidate
    ffcm_notification_messagesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ffcm_notification_messagesedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("ffcm_notification_messagesedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="ffcm_notification_messagesedit" id="ffcm_notification_messagesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="fcm_notification_messages" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_fcm_notification_messages_user_id">
    <select
        ...
        data-select2-id="ffcm_notification_messagesedit_x_user_id"
...
<script>
loadjs.ready("ffcm_notification_messagesedit", function() {
    var options = { name: "x_user_id", selectId: "ffcm_notification_messagesedit_x_user_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffcm_notification_messagesedit.lists.user_id.lookupOptions.length) {
        options.data = { id: "x_user_id", form: "ffcm_notification_messagesedit" };
    } else {
        options.ajax = { id: "x_user_id", form: "ffcm_notification_messagesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["ffcm_notification_messagesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffcm_notification_messagesedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["ffcm_notification_messagesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffcm_notification_messagesedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
