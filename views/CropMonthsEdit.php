...
var fcrop_monthsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcrop_monthsedit = new ew.Form("fcrop_monthsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcrop_monthsedit;

    // Add fields
    var fields = currentTable.fields;
    fcrop_monthsedit.addFields([
    ...
    // Form_CustomValidate
    fcrop_monthsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrop_monthsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcrop_monthsedit.lists.crop_id = <?= $Page->crop_id->toClientList($Page) ?>;
    fcrop_monthsedit.lists.sow_under_cover_from = <?= $Page->sow_under_cover_from->toClientList($Page) ?>;
    fcrop_monthsedit.lists.sow_under_cover_to = <?= $Page->sow_under_cover_to->toClientList($Page) ?>;
    fcrop_monthsedit.lists.sow_direct_from = <?= $Page->sow_direct_from->toClientList($Page) ?>;
    fcrop_monthsedit.lists.sow_direct_to = <?= $Page->sow_direct_to->toClientList($Page) ?>;
    fcrop_monthsedit.lists.plant_start_month = <?= $Page->plant_start_month->toClientList($Page) ?>;
    fcrop_monthsedit.lists.plant_end_month = <?= $Page->plant_end_month->toClientList($Page) ?>;
    fcrop_monthsedit.lists.harvest_start_month = <?= $Page->harvest_start_month->toClientList($Page) ?>;
    fcrop_monthsedit.lists.harvest_end_month = <?= $Page->harvest_end_month->toClientList($Page) ?>;
    loadjs.done("fcrop_monthsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcrop_monthsedit" id="fcrop_monthsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="crop_months" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_crop_months_crop_id">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_crop_id"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_crop_id", selectId: "fcrop_monthsedit_x_crop_id" };
    if (fcrop_monthsedit.lists.crop_id.lookupOptions.length) {
        options.data = { id: "x_crop_id", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_crop_id", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_crop_months_sow_under_cover_from">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_sow_under_cover_from"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_sow_under_cover_from", selectId: "fcrop_monthsedit_x_sow_under_cover_from" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.sow_under_cover_from.lookupOptions.length) {
        options.data = { id: "x_sow_under_cover_from", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_sow_under_cover_from", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_crop_months_sow_under_cover_to">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_sow_under_cover_to"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_sow_under_cover_to", selectId: "fcrop_monthsedit_x_sow_under_cover_to" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.sow_under_cover_to.lookupOptions.length) {
        options.data = { id: "x_sow_under_cover_to", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_sow_under_cover_to", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_sow_direct_from">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_sow_direct_from"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_sow_direct_from", selectId: "fcrop_monthsedit_x_sow_direct_from" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.sow_direct_from.lookupOptions.length) {
        options.data = { id: "x_sow_direct_from", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_sow_direct_from", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_sow_direct_to">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_sow_direct_to"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_sow_direct_to", selectId: "fcrop_monthsedit_x_sow_direct_to" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.sow_direct_to.lookupOptions.length) {
        options.data = { id: "x_sow_direct_to", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_sow_direct_to", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_plant_start_month">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_plant_start_month"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_plant_start_month", selectId: "fcrop_monthsedit_x_plant_start_month" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.plant_start_month.lookupOptions.length) {
        options.data = { id: "x_plant_start_month", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_plant_start_month", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_plant_end_month">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_plant_end_month"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_plant_end_month", selectId: "fcrop_monthsedit_x_plant_end_month" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.plant_end_month.lookupOptions.length) {
        options.data = { id: "x_plant_end_month", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_plant_end_month", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_harvest_start_month">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_harvest_start_month"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_harvest_start_month", selectId: "fcrop_monthsedit_x_harvest_start_month" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.harvest_start_month.lookupOptions.length) {
        options.data = { id: "x_harvest_start_month", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_harvest_start_month", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_crop_months_harvest_end_month">
    <select
        ...
        data-select2-id="fcrop_monthsedit_x_harvest_end_month"
        data-table="crop_months"
...
<script>
loadjs.ready("fcrop_monthsedit", function() {
    var options = { name: "x_harvest_end_month", selectId: "fcrop_monthsedit_x_harvest_end_month" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcrop_monthsedit.lists.harvest_end_month.lookupOptions.length) {
        options.data = { id: "x_harvest_end_month", form: "fcrop_monthsedit" };
    } else {
        options.ajax = { id: "x_harvest_end_month", form: "fcrop_monthsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fcrop_monthsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_monthsedit", "x_updated_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fcrop_monthsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcrop_monthsedit", "x_created_at", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
