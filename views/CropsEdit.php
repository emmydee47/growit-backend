...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcropsedit = new ew.Form("fcropsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcropsedit;

    // Add fields
    var fields = currentTable.fields;
    fcropsedit.addFields([
    ...
    // Form_CustomValidate
    fcropsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcropsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcropsedit.lists.grow_level = <?= $Page->grow_level->toClientList($Page) ?>;
    fcropsedit.lists.category = <?= $Page->category->toClientList($Page) ?>;
    fcropsedit.lists.life_cycle = <?= $Page->life_cycle->toClientList($Page) ?>;
    fcropsedit.lists.crop_cover_image = <?= $Page->crop_cover_image->toClientList($Page) ?>;
    loadjs.done("fcropsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcropsedit" id="fcropsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" name="fa_x_media_url" id= "fa_x_media_url" value="<?= (Post("fa_x_media_url") == "0") ? "0" : "1" ?>">
...
<span id="el_crops_category">
    <select
        ...
        data-select2-id="fcropsedit_x_category"
        data-table="crops"
...
<script>
loadjs.ready("fcropsedit", function() {
    var options = { name: "x_category", selectId: "fcropsedit_x_category" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcropsedit.lists.category.lookupOptions.length) {
        options.data = { id: "x_category", form: "fcropsedit" };
    } else {
        options.ajax = { id: "x_category", form: "fcropsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_crops_life_cycle">
    <select
        ...
        data-select2-id="fcropsedit_x_life_cycle"
        data-table="crops"
...
<script>
loadjs.ready("fcropsedit", function() {
    var options = { name: "x_life_cycle", selectId: "fcropsedit_x_life_cycle" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcropsedit.lists.life_cycle.lookupOptions.length) {
        options.data = { id: "x_life_cycle", form: "fcropsedit" };
    } else {
        options.ajax = { id: "x_life_cycle", form: "fcropsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->crop_cover_image->isInvalidClass() ?>" data-table="crops" data-field="x_crop_cover_image" name="x_crop_cover_image[]" id="x_crop_cover_image_665475" value="1"<?= ConvertToBool($Page->crop_cover_image->CurrentValue) ? " checked" : "" ?><?= $Page->crop_cover_image->editAttributes() ?> aria-describedby="x_crop_cover_image_help">
